...
export class NightlyConnectIotaAdapter extends EventEmitter<IotaAdapterEvents> {
  ...
  private _app: AppIota | undefined
  private _innerStandardAdapter:
    | WalletWithFeatures<
        StandardConnectFeature &
          StandardEventsFeature &
          IotaFeatures &
  ...
  constructor(appInitData: AppInitData, connectionOptions?: ConnectionOptions) {
    ...
    if (!this._appInitData.persistent) {
      clearSessionIdForNetwork(IOTA_NETWORK)
    }
  }

  public static initApp = async (
    appInitData: AppInitData
  ): Promise<[AppIota, WalletMetadata[]]> => {
    try {
      return await Promise.all([
        AppIota.build(appInitData),
        AppIota.getWalletsMetadata(
  ...
  public static build = async (
    ...
    try {
      ...
      const [app, metadataWallets] = await NightlyConnectIotaAdapter.initApp(appInitData)
      ...
      app.on('userConnected', async (e) => {
        try {
          persistRecentWalletForNetwork(IOTA_NETWORK, {
  ...
  public static buildLazy = (
    ...
    if (!adapter._connectionOptions.initOnConnect) {
      ...
      NightlyConnectIotaAdapter.initApp(appInitData)
    ...
    return new Promise<void>((resolve, reject) => {
      const innerConnect = async () => {
        try {
          ...
          const recentWallet = getRecentWalletForNetwork(IOTA_NETWORK)
            ...
            if (recentWallet.walletType === ConnectionType.Nightly) {
              if (this._app?.hasBeenRestored() && this._app.connectedPublicKeys.length > 0) {
                ...
                try {
                  ...
                  const [app] = await NightlyConnectIotaAdapter.initApp(this._appInitData)
                  this._app = app
                }
              }
            }
          ...
          if (this._connectionOptions.initOnConnect) {
            this._loading = true
            NightlyConnectIotaAdapter.initApp(this._appInitData)
              .then(([app, metadataWallets]) => {
                this._app = app
    ...
    if (this.connected) {
      switch (this._connectionType) {
        case ConnectionType.Nightly: {
          clearSessionIdForNetwork(IOTA_NETWORK)
          // Refresh app session
          this._app = await AppIota.build(this._appInitData)
  ...
  connectToStandardWallet = async (walletName: string) => {
    try {
      ...
      const adapter = wallet.standardWallet as WalletWithFeatures<
        StandardConnectFeature &
          StandardEventsFeature &
          IotaFeatures &
          Partial<StandardDisconnectFeature>
      ...
      this._accounts = adapter.accounts.map((a) => a)
      this.connected = true
      this._connecting = false
      this.connecting = false
      this.emit('connect', this._accounts)
      ...
      adapter.features['standard:events'].on('change', (a) => {
        this.emit('change', a)
      })
  ...
  fetchWalletsFromRegistry: () => Promise<WalletMetadata[]> = async () => {
    this._metadataWallets = await AppIota.getWalletsMetadata(
...
export const createSuiWalletAccountFromString = (publicKey: string): WalletAccount => {
  ...
  return {
    ...
    publicKey: iotaPk.toRawBytes(),
    chains: ['iota:devnet', 'iota:testnet', 'iota:mainnet'],
    features: [
      'standard:connect',
      'standard:events',
