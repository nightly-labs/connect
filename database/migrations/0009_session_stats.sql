----------------- Sessions Stats -----------------
--- View
CREATE MATERIALIZED VIEW sessions_stats_per_app_daily WITH (timescaledb.continuous) AS
SELECT
    app_id,
    ...
    ) AS avg_daily_session_duration_seconds
FROM
    sessions
WHERE
...
ALTER MATERIALIZED VIEW sessions_stats_per_app_daily
SET
    (timescaledb.materialized_only = false);

----------------- Monthly Average session duration -----------------
--- View
CREATE MATERIALIZED VIEW sessions_stats_per_app_monthly WITH (timescaledb.continuous) AS
SELECT
    app_id,
...
--- Real time aggregation
ALTER MATERIALIZED VIEW sessions_stats_per_app_monthly
SET
    (timescaledb.materialized_only = false);